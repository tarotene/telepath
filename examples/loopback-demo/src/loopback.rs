...
//!
//! One end implements `telepath_server::transport::Transport` (non-blocking),
...
pub struct ServerSideTransport {
...
pub struct ClientSideTransport {
    rx: Receiver<u8>,   // fw   → host
    tx: SyncSender<u8>, // host → fw
}

/// Create a connected (`ServerSideTransport`, `ClientSideTransport`) pair.
pub fn make_pair() -> (ServerSideTransport, ClientSideTransport) {
...
impl Transport for ServerSideTransport {
...
impl std::io::Read for ClientSideTransport {
...
impl std::io::Write for ClientSideTransport {
